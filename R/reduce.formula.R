...
reduce.existing.formula <- function(dt, the.initial.formula, max.input.categories = 20, max.outcome.categories.to.search = 4, return.as = "formula"){
  require(data.table)
  dt <- setDT(dt)
  ...
  return(reduce.formula(dt = dt, outcome.name = lhs.original, input.names = the.pieces.in.names, input.patterns = NA, return.as = return.as))
